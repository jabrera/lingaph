...
<div id="container">
	...
	<div id="content">
		<div id="content_base">
			<div id="center"><div id="pad">
				<?php
				if(isset($_GET['read'])) {
					echo '<span class="button"><a href="news.php" class="button">See All News</a></span>';
					$news_id = $_GET['read'];
					...
					while($row=mysql_fetch_array($query)) {
						echo '<div id="title">
						<span>'.$row['Title'].'</span>
						<div>'.$row['DatePosted'].' - '.$row['TimePosted'].'</div>
					</div>
					<p>
					'.nl2br($row['Message']).'
					</p>
	';
					...
					<p>
					News is either missing or not available.
					</p>
	';
					}
				} else {
	...
	';
					...
					<p>
					No news posted in this website.
					</p>
	';
					}
				}
